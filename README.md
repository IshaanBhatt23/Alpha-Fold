...
We provide a simple graphical user interface to run DiffDock on a single complex. To use it, run the following commands:

    1. docker run -it --rm --gpus all -p 7860:7860 rbgcsail/diffdock bash
    2. eval "$(~/bin/micromamba shell hook -s bash)"
    3. micromamba activate diffdock
    4. python app/main.py --server_name 0.0.0.0
